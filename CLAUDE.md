...
商店有三类条目：`tpl`（ClawTpl 出厂模板）、`mod`（ClawMod 资产包）、
`ref`（开源引用）。前两者托管源文件，`ref` 只是指向上游仓库的引用。
...
| 层 | 路径 | 职责 |
|---|---|---|
| 数据 | `registry/{tpl,mod,ref}/<id>/` | 条目源 —— `claw.json` + 可选 `README.md` + `files/` |
...
- 条目 `id` 必须与文件夹名一致；约定前缀 `clawtpl-` / `clawmod-` / `ref-`。
